...
function createApp(options = {}) {
  ...
  // ── YouTube IFrame player host page ───────────────────────────────────────
  // GET /player?videoId=<id>
  //
  // The Android app loads this page into its WebView instead of injecting HTML
  // with `loadDataWithBaseURL`. That matters: injected content has a synthetic
  // origin and sends no Referer, which YouTube's embed checks reject with the
  // 150-series error ("playback on other websites has been disabled") even for
  // videos whose owners *do* allow embedding. Serving the page over real HTTP
  // gives the iframe a genuine origin and referer, so the embed is accepted.
  //
  // Player commands are plain JS functions called from Kotlin via
  // `evaluateJavascript`; events are pushed back through the `AndroidBridge`
  // JavaScript interface. `AndroidBridge` only exists inside the app's WebView,
  // so every call is guarded for when the page is opened in a normal browser.
  app.get('/player', (req, res) => {
    const videoId = (req.query.videoId ?? '').trim();
    if (videoId !== '' && !isValidYouTubeVideoId(videoId)) {
      res.status(400).type('text/plain').send('Invalid videoId');
      return;
    }
    // Embedded in a JSON string literal so the value cannot break out of the
    // script context, on top of the strict ID validation above.
    const encodedId = JSON.stringify(videoId);
...
<script>
  var ytPlayer;
  var initialVideoId = ${encodedId};

  function bridgeCall(name, arg) {
    try {
      if (typeof AndroidBridge !== 'undefined' && AndroidBridge[name]) {
        if (arg === undefined) AndroidBridge[name](); else AndroidBridge[name](arg);
      }
    } catch (e) {}
  }
  ...
  function onYouTubeIframeAPIReady() {
    ytPlayer = new YT.Player('player', {
      ...
      events: {
        onReady:       function(e) { bridgeCall('onReady'); },
        onStateChange: function(e) { bridgeCall('onStateChange', e.data); },
        onError:       function(e) { bridgeCall('onError', e.data); }
      }
    });
  }
  ...
  setInterval(function() {
    try {
      if (ytPlayer && ytPlayer.getCurrentTime) {
        ...
        if (curr !== undefined && curr !== null) bridgeCall('onCurrentTime', curr);
        if (dur !== undefined && dur !== null) bridgeCall('onDuration', dur);
      }
    } catch (ignored) {}
  }, 500);
</script>
